...
@Controller
public class MapMeterLocationController {
	@Autowired
	private LocationUtilService restService;
	@Autowired
	private SubstationDataServiceImpl substationDataService;
	@Autowired
	private MapMeterLocationService mapMeterLocationService;
	@Autowired
	...
	public String substationMaster(@RequestParam(value = "circleSelected") Integer circleCode,
		...
		model.addAttribute("substationList", mapMeterLocationService.findSubstationEnergyMeters(locationModel));
	...
	public ModelAndView saveMeterLocationMapping(LocationMeterMappingModel locationMeterMap, BindingResult bindingResult,
		...
		String error = mapMeterLocationService.validateDates(locationMeterMap);

		LocationMeterMappingModel resultModel=mapMeterLocationService.saveLocationMeterMapping(locationMeterMap);
		if (error == null && resultModel.getMappingSuccesful()) {
			List<MapMeterLocation> mappings = restService.findLocations(locationMeterMap.getMeterMaster());
	...
	public ModelAndView changeMeterLocation(@RequestParam(value = "meterlocationId") int meterlocationId,
		...
		chgMtr.setOldValues(mapMeterLocationService.getMeterDetails(meterlocationId));
		model.addAttribute("error", "");
		model.addAttribute("changeMeterSnippet", chgMtr);
		List<MapMeterLocation> mappings = restService.findLocations(chgMtr.getOldMeterLocationMap().getMeterMaster(),chgMtr.getOldMeterLocationMap().getLocationMaster());
		model.addAttribute("locationMeterMappingList", mappings);
	...
	public ModelAndView saveRemovedMeterLocationMapping(LocationMeterMappingModel locationMeterMap, BindingResult bindingResult,
		...
		ModelAndView modelAndView=null;
		String error = mapMeterLocationService.validateEndDate(locationMeterMap);

		LocationMeterMappingModel resultModel=mapMeterLocationService.endLocationMeterMapping(locationMeterMap);
	...
	public ModelAndView mapLocation(@RequestParam(value = "locationid") String locationid, ModelMap model) {
		...
		List<MapMeterLocation> mappings = restService.findLocations(locationMeterMappingModel.getMeterMaster());
		model.addAttribute("locationMeterMappingList", mappings);
